...
namespace Sass {
  ...
  Node Document::parse_import()
  {
    ...
    resolvedPath = import_path;
    for (vector<string>::iterator path = context.include_paths.begin(); path < context.include_paths.end(); ++path) {
      try {
        resolvedPath = *path + import_path;
        Document importee(Document::make_from_file(context, resolvedPath));
        importee.parse_scss();
    ...
    throw_read_error("error reading file \"" + import_path + "\" from \"" + resolvedPath +"\"");
    // unreached statement
    return Node();
  }
